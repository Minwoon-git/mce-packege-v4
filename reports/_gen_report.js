...
// ---- design tokens (report-guide §7 SSOT) ----
...
const meta = [["모수", "100,000명"], ["기준일", "2026.07.22"], ["업종", "일반 이커머스"]];
meta.forEach(([k, v], i) => {
  const x = MX + i * 2.7;
...
rows.push([
  { text: "모수 (전체)", options: { fontFace: F, fontSize: 11, bold: true, color: INK, fill: { color: WHITE }, border: [{ pt: 2, color: INK }, { pt: 0.5, color: LINE }, { pt: 0.5, color: LINE }, { pt: 0.5, color: LINE }] } },
  { text: "100,000", options: { fontFace: F, fontSize: 11, bold: true, color: INK, align: "right", fill: { color: WHITE }, border: [{ pt: 2, color: INK }, { pt: 0.5, color: LINE }, { pt: 0.5, color: LINE }, { pt: 0.5, color: LINE }] } },
  { text: "—", options: { fontFace: F, fontSize: 11, bold: true, color: INK, align: "right", fill: { color: WHITE }, border: [{ pt: 2, color: INK }, { pt: 0.5, color: LINE }, { pt: 0.5, color: LINE }, { pt: 0.5, color: LINE }] } },
]);
...
footnote(s, "* 1회성·이탈위험은 구매자(85,059명) 대비, 그 외는 전체(100,000명) 대비 · RECON_Profile 집계 · 기준일 2026-07-22");
...
const opp = [
  ["1", "2차 구매 유도", "1회성 64.7%", "55,034", "재구매율·LTV 상승 (최대 모수)", true],
  ["2", "이탈 고객 재구매 유도", "이탈위험 32.6%", "27,729", "이탈 방어·매출 회복", true],
  ["3", "휴면 고객 재활성화", "휴면 32.3%", "32,255", "재방문 회복·잠재 이탈 차단", true],
  ["4", "동의 확보", "미동의 19.9%", "19,893", "도달 채널 확보 (전 캠페인 기반)", false],
  ["5", "장바구니 리마인더", "장바구니 18.1%", "18,149", "결제 완주·단기 전환", false],
  ["6", "신규 첫구매 유도", "미전환 14.9%", "14,941", "첫 전환·신규 활성화", false],
];
const orows = [oh];
opp.forEach(([r, c, b, d, im, top], i) => {
  const fill = top ? "F1F7F6" : (i % 2 ? "FAFBFC" : WHITE);
  orows.push([
    { text: r, options: { fontFace: F, fontSize: 11, bold: true, color: top ? ACCENT : MUTE, align: "center", fill: { color: fill } } },
    { text: c, options: { fontFace: F, fontSize: 11.5, bold: true, color: INK, fill: { color: fill } } },
    { text: b, options: { fontFace: F, fontSize: 11, color: TEXT, fill: { color: fill } } },
    { text: d, options: { fontFace: F, fontSize: 11, color: TEXT, align: "right", fill: { color: fill } } },
    { text: im, options: { fontFace: F, fontSize: 11, color: TEXT, fill: { color: fill } } },
  ]);
});
s.addTable(orows, { x: MX, y: 1.85, w: W - 2 * MX, colW: [0.7, 3.1, 2.2, 1.6, 4.33], rowH: 0.5, valign: "middle", border: { pt: 0.5, color: LINE } });
s.addText([
  { text: "상위 3개(2차 구매·이탈·휴면)만으로 ", options: {} },
  { text: "연인원 11만 건 이상", options: { bold: true, color: INK } },
  { text: "의 접점이 나오며, 모두 이미 확보한 고객이라 신규 획득 대비 전환 효율이 높습니다. 4번 ", options: {} },
  { text: "동의 확보", options: { bold: true, color: INK } },
  { text: "는 미동의 2만 명을 살려 이후 모든 캠페인의 도달률을 끌어올리는 기반 캠페인입니다.", options: {} },
], { x: MX, y: 5.75, w: W - 2 * MX, h: 0.9, fontFace: F, fontSize: 13, color: TEXT, lineSpacing: 20 });
...
const camps = [
  ["01", "2차 구매 유도", "55,034명 · 1회성 구매자", "이메일 · 알림톡 (동의 필터)", "첫 구매 연관·재입고 추천, 재구매 인센티브(쿠폰·적립)", "재구매율↑ · LTV↑"],
  ["02", "이탈 고객 재구매 유도", "27,729명 · 주문 90일+ 경과", "이메일 + 복귀 쿠폰", "개인화 리마인드, 복귀 한정 혜택으로 재구매 유도", "이탈 방어 · 매출 회복"],
  ["03", "휴면 고객 재활성화", "32,255명 · 로그인 90일+ 미접속", "이메일 · 알림톡", "관심 카테고리 기반 재방문 유도, 복귀 트리거", "재방문·재활성화율 개선"],
];
...
camps.forEach(([no, name, tgt, ch, msg, imp], i) => {
  ...
  const kv = [["대상", tgt], ["채널", ch], ["메시지 방향", msg]];
  let yy = 3.15;
  kv.forEach(([k, v]) => {
    const th = k === "메시지 방향" ? 0.72 : 0.34;
...
s.addText("방법론. 마스터 고객 데이터(RECON_Profile, RAW_* 5테이블 JOIN 집계 · 1행=1고객)를 세그먼트별 집계해 산출. 데이터 기준일 2026-07-22.\n* 1회성·이탈위험 비율은 구매자(85,059명) 대비, 그 외 지표는 전체(100,000명) 대비. 기준일수(휴면·이탈 90일)는 활성 고객사 분석 가이드 §2 정의값 가정이며 조정 가능.",
  { x: MX, y: 6.05, w: W - 2 * MX, h: 0.8, fontFace: F, fontSize: 10, color: "8B93A8", lineSpacing: 15 });
// (기준일 2026-07-22 · 휴면·이탈 90일 기준선 = 활성 고객사 분석 가이드 §2 정의값 가정)
