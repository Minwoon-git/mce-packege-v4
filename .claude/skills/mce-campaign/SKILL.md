...
- **스키마 매핑 (STEP 0)** → [`reference/schema-mapping.md`](reference/schema-mapping.md) — 고객 스키마 파일(DDL/CSV)을 표준 스키마로 매핑하고 RAW DE·Import·가이드 MD를 세팅할 때(STEP 0 SSOT)
...
```
(신규 고객사 온보딩 시에만 — 원천 데이터를 처음 붙일 때)
[STEP 0] 스키마 분석  →  고객 스키마 파일(DDL/CSV) 분석 → 표준 매핑
   ...
   │        Phase B: 빈 RAW DE 생성 + SFTP→DE Import 세팅 + 활성 고객사 가이드 MD 자동생성
...
[STEP 1] 주제 선정  →  연결된 DE 분석 → 캠페인 후보 목록 추천
   │        → (리스트업/진단이면) 분석 리포트(PPT) 자동 생성 → 파일 경로 제시
...
> ⓪ **STEP 0(스키마 분석)은 신규 고객사 온보딩 시에만** 수행한다 — 고객 원천 데이터를 이 패키지로 처음 붙일 때 1회. 이미 활성 고객사 가이드 + RAW DE가 갖춰진 계정에서는 건너뛴다. 상세 절차·표준 스키마·매핑 규칙·HITL 항목은 [`reference/schema-mapping.md`](reference/schema-mapping.md)가 SSOT이고, 실작업은 `mce-schema-agent` 워커에 위임한다(2-페이즈: 분석 → HITL → materialize). **STEP 0의 산출물은 매핑표(구조) + 빈 RAW DE + Import + 가이드 MD**까지이며, 분석 리포트(PPT)는 데이터 적재 후 STEP 1에서 나온다.
...
> 고객마다 파일명·컬럼명이 제각각인 원천 데이터를 **표준 스키마로 매핑**하고, **빈 RAW DE + SFTP Import + 활성 고객사 가이드 MD**를 세팅해 STEP 1이 그대로 돌 수 있게 만드는 **앞단**이다. **상세 절차·표준 스키마·매핑 규칙·HITL 항목·가드레일의 SSOT = [`reference/schema-mapping.md`](reference/schema-mapping.md).** 실작업은 `mce-schema-agent` 워커에 위임한다.
>
> ⭐ **표준 스키마가 "계약"이다.** STEP 0의 출력이 항상 표준 이름의 RAW DE + 표준 규칙의 가이드 MD이므로 하류(STEP 1~4)는 무수정으로 재사용된다. STEP 0는 지금 사람이 손으로 쓰던 활성 고객사 가이드 MD를 **자동 생성**하는 일이다.
> ⭐ **분석 대상은 "구조(스키마)"** 다 — 데이터 없이도 동작한다. **값 분석·캠페인 추천·분석 리포트(PPT)는 데이터 적재 후 STEP 1**에서 나온다.

## 0-1. 언제 하나
- 신규 고객사 온보딩(원천 데이터 최초 연결) 1회. 또는 사용자가 스키마 파일(DDL/CSV)을 첨부하며 "스키마 분석/매핑/이 데이터 붙여줘"라고 할 때.
- 캠페인 요청인데 **활성 고객사 가이드가 없거나 원천 RAW DE가 비어 있으면**, 오케스트레이터가 STEP 0를 먼저 태운다.
- 이미 활성 고객사 가이드 + RAW DE가 갖춰진 계정에서는 **건너뛴다**(재실행 금지, 스키마 변경 시만).

## 0-2. 오케스트레이터 진행 (2-페이즈 + 사이 HITL)

STEP 0의 HITL(핵심 컬럼 확인)은 워커가 격리 실행이라 직접 못 묻는다. **오케스트레이터가 워커 호출 "사이"에서** `AskUserQuestion`으로 받는다(채널 해소와 동일 패턴).

1. **Phase A 위임** — `mce-schema-agent`에 스키마 파일 경로/내용을 넘겨 호출 → 워커가 **매핑표(고객 컬럼→표준 컬럼)·조인키·값변환 규칙 + HITL 확인 필요 목록**을 반환한다. (DE/Import/MD는 아직 안 만듦 = **산출물 ① 매핑표**, 데이터 없이 나옴)
2. **핵심 컬럼 확인(HITL)** — 오케스트레이터가 워커가 올린 항목을 `AskUserQuestion`으로 사용자에게 확정받는다. 최소:
   - **핵심 ID(조인키)** — 전 테이블을 잇는 회원 식별자 확정
   - **총구매액 산식** — `order_amount` 합 vs 상세 `price×quantity` 합 / **취소·환불(`order_status`) 제외 여부**
   - **날짜 기준** — `last_order_date`·`last_login_date`가 어느 컬럼인지
   - **동의값 해석** — `Y/N`·공란을 동의/미동의로 어떻게 볼지
   - 신뢰도 낮은 매핑 전부
3. **Phase B 위임** — 확정 매핑 + HITL 확정값 + 고객사명을 `mce-schema-agent`에 넘겨 다시 호출 → 워커가 **빈 RAW DE 생성 + SFTP Import 세팅 + `analysis-guide/<고객사>.md` 자동 생성**을 수행하고 결과를 반환한다.
...
5. **데이터 적재 게이트 안내** — 세팅 완료 후 "고객이 SFTP에 파일 업로드 → Import 적재 → 그때 STEP 1 진단/리포트 가능"임을 보고한다. STEP 0는 적재를 **세팅만** 한다(즉시 실행 안 함).
...
**추천 캠페인 — 우선순위**
1️⃣ **2차 구매 유도** ← 1회성 구매자 64% (6,400명)
2️⃣ **휴면 고객 재활성화** ← 휴면 35% (3,500명)
3️⃣ **이탈 고객 재구매 유도** ← 이탈위험 32% (2,528명)
> 추가 가능: 생일 쿠폰(birthday), 쿠폰 만료 리마인더(coupon_expire_date), 등급/취향/지역 분기 ...

만들고 싶은 캠페인을 골라주세요. 번호를 고르시거나 `추천`이라고 입력하시면 의도에 맞는 후보를 골라드립니다.
```

- **목록 표기**: 각 추천은 `숫자 이모지 + **캠페인명(볼드)** ← 지표 비율 (인원)` 형식으로 쓴다. 숫자 이모지는 1️⃣~🔟을 순위대로 쓰고(10위 초과만 `11.` 평문 폴백), "추가 가능…" 줄은 `>` 인용으로 흐리게 둔다. 위 3줄은 예시일 뿐 실제 추천 개수만큼 이어서 낸다.
...
> 📄 **분석 리포트 자동 생성 (갈래 A 진단 직후)**: 리스트업/진단 결과가 나오면, [`reference/report-guide.md`](reference/report-guide.md) §6·§7대로 **진단 데이터 JSON을 작성해 빌더로 PPTX 리포트를 생성**한다(`node reference/report-builder/gen_report.js <데이터.json> <출력.pptx>` — 디자인·레이아웃은 빌더에 고정, AI는 데이터 JSON만 작성). `reports/`에 저장하고 **파일 경로를 후보 목록과 함께 제시**한다. 리포트엔 핵심 요약·고객 기반 구조(퍼널/도달)·세그먼트 진단·기회 우선순위·캠페인 상세가 담긴다(구조·디자인은 공통, 값은 이번 진단·활성 고객사 분석 가이드 기준). **HTML/Artifact는 생성하지 않으며, 슬라이드 코드를 매번 새로 작성하지 않는다.** 캠페인 선택 전 단계다.
> 🚫 **무발화**: 워커 위임→진단→리포트 생성은 **전부 침묵하며 연속 실행**한다. "위임했습니다 / 완료됐습니다 / 갈래 A이므로 생성하겠습니다 / 템플릿 값과 일치합니다 / 리포트를 생성하겠습니다" 같은 진행 멘트를 도구 호출 사이에 **한 줄도** 넣지 않고, **맨 마지막에 진단표 + 추천 목록 + 리포트 파일 경로를 1회에** 출력한다(전역 "🚫 결과만 전달" 규칙).
