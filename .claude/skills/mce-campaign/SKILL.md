...
> 📄 **분석 리포트 자동 생성 (갈래 A 진단 직후)**: 리스트업/진단 결과가 나오면, [`reference/report-guide.md`](reference/report-guide.md) §7대로 **PPTX 리포트**(`report-template.html`의 구조·디자인 토큰 이식 + 진단 데이터 주입)를 `reports/`에 생성하고 **파일 경로를 후보 목록과 함께 제시**한다. 리포트엔 진단·기회 우선순위·핵심 캠페인 상세가 담긴다(구조·디자인은 공통, 값은 이번 진단·활성 고객사 분석 가이드 기준). **HTML/Artifact는 생성하지 않는다.** 캠페인 선택 전 단계다.
