...
|---|---|---|---|
| **A. 의도 없이 전체 (리스트업)** *(우선 갈래)* | "생성 가능한 캠페인 리스트 업", "어떤 캠페인 만들 수 있어?", "전체 보여줘", "캠페인 목록" 등 **특정 의도 키워드가 없는 포괄적 요청** | `Customer_Profile`(key `CD_Customer_Profile_DE`)의 **필드(원천 신호 컬럼) 목록만** (`sfmc_get_data_extension_fields`, 행 집계 안 함) | **"캠페인명 (신호 컬럼)" 형태의 가능 캠페인 목록**을 번호로 제시 (진입 DE 나열·대상자 집계 없음, → 1-4-A) |
| **B. 의도 포함** | "신규 회원 캠페인 만들어줘", "이탈 고객 캠페인", "장바구니 캠페인" 등 **신규/이탈/장바구니/생일/쿠폰 등 의도 키워드 포함** | `Customer_Profile`의 해당 의도 신호 컬럼 위주 | 해당 신호의 **상세 후보 표** (복잡도 단순→복합 정렬, → 1-4-B) |

> ⚠️ **갈래 A는 "캠페인"을 나열한다 — "진입 DE"를 나열하지 않는다.** `Customer_Profile`에 존재하는 **원천 컬럼**으로 만들 수 있는 캠페인을 [`reference/de-and-folders.md`](reference/de-and-folders.md)의 "원천 컬럼 → 추천 캠페인" 표 기준으로 제시한다. `Campaign_Package` 하위 진입 DE(`WELCOME_ENTRY_DE` 등)나 계정에 떠 있는 기존 진입 DE 목록을 긁어 나열하지 않는다. (진입 DE는 캠페인을 **고른 뒤** 1-6에서 Automation SQL Query로 생성한다.)
...
## 1-1. 고객 데이터 분석 — `Customer_Profile` 를 읽는다 (먼저 수행)

> 추천은 **고객 데이터 분석에 근거**한다. 분석 소스는 `Customer_Profile`(key `CD_Customer_Profile_DE`) 하나다.
> 진입 DE 폴더 구조·신호 컬럼 매핑·fallback 절차는 [`reference/de-and-folders.md`](reference/de-and-folders.md)에 정리돼 있다.

`Customer_Profile`을 읽어(SQL Query 또는 행 조회) **각 캠페인 신호에 해당하는 고객 수(세그먼트 규모)** 를 집계한다.
...
### 1-4-A. 갈래 A (의도 없이 전체 = 리스트업) — 컬럼 기반 "생성 가능한 캠페인" 목록

`Customer_Profile`의 **컬럼(신호)을 보고 만들 수 있는 캠페인을 쫙 나열**한다.
> ⚡ **이 단계에서는 대상자 수를 집계하지 않는다**(가볍게). 대상자 집계는 사용자가 캠페인을 **고른 뒤**(1-6)에 한다.
> 즉 "이 신호 컬럼이 있으니 이런 캠페인이 가능하다"만 보여준다. `Customer_Profile`의 필드 목록만 확인하면 충분하다(`sfmc_get_data_extension_fields`).
...
만들고 싶은 캠페인을 골라주세요. 선택하시면 대상자 수를 집계하고 진입 DE를 만듭니다.
```

- 해당 신호 컬럼이 Profile에 **없으면** 그 캠페인은 목록에서 제외한다(컬럼 존재 = 가능성).
