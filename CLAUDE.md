...
사용자 한 문장
   ...
   │
   ├─ (리스트업/진단이면) 상위가 분석 리포트(PPT) 자동 생성 → reports\ 저장 → 파일 경로 제시  [reference/report-guide.md §7]  (HTML/Artifact 미생성)
...
> ⚠️ **위임(`Agent` 호출)·리포트(PPT) 생성도 "도구 호출"이다** — 그 전후·사이에 위 같은 멘트를 절대 넣지 않는다. STEP 1 갈래 A(리스트업)는 워커 위임→진단→PPT 리포트 생성을 **전부 무발화로 연속 실행**하고, **맨 마지막에 진단표+추천 목록+리포트 파일 경로를 1회에 출력**한다(그 사이 "위임했습니다/완료됐습니다/일치합니다/생성하겠습니다"류 한 줄도 금지).
